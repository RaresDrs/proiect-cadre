...
while i < len(lines):
    ...
    if line.strip() == 'if modul == "Calcul Grinzi simplu":':
        ...
        new_lines.append('    col_panel, col_canvas = st.columns([1.4, 3.6], gap="large")')
    ...
    if 'st.session_state.gv_forces=f_edited' in line:
        ...
        new_lines.append(" " * (spaces + 8) + line.lstrip())
        ...
        while i < len(lines):
            ...
            if rest_line.strip() == "":
                ...
                spaces = len(rest_line) - len(rest_line.lstrip())
                new_lines.append(" " * (spaces + 4) + rest_line.lstrip())
            i += 1
